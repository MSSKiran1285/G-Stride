...
| **BL-044 divergence** | **Ratified 16 Aug 2026 by the owner** — "I am fine with the way it is currently working, no action needed." The Process area control stays a dropdown with a "+ New process area" option rather than the combobox the criteria word; the intent (see and pick from existing areas) is met. Closed. |
| **Automation run reference — chain scope** | **Fixed 16 Aug 2026.** `CreateAutomationRunReference` is now idempotent within a run: the first stage creates the reference and later stages keep it, so a Process shares ONE owner-linked reference instead of minting one per Test. Before the guard a three-Test Process produced three unrelated identifiers in the evidence that exists to correlate it — reachable the moment every transactional Test was given its own step. Covered by `regression/automation-run-reference.test.js`. |
...
| **Compose redesign** | **Closed 16 Aug 2026 by the owner** — "we can close it for now, I still need some improvements, but will circle back". Not taken. Two observed runs against modelled floors: the 16-step Sales Order build **25 min vs 5m53s**, the 7-step Purchase Order build **~6 min vs 3m26s** — the second after the step-form work, same author, same checklist protocol. Traces and both observed runs are in `regression/results/compose-timing/`. |
| **Step form streamlining** | Done 14–16 Aug 2026 across several owner reviews. `ModuleParamDescriptor` gained `type` / `options` / `advanced` / `default`; defaults soft-fill rather than showing ghost text; the editor is a pop-out; fields are label-left, one per row; and source-plus-value merged into a single `ValuePicker` after an observed run saved dataset column names as literals. Latest modelled floors: **Sales Order 163 acts / 6m14s**, **Purchase Order 91 acts / 3m25s** — more interactions than before, same time, bought for correctness and headroom rather than speed. |
